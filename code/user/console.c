...
void* load( char* x ) {
  ...
  else if (0 == strcmp( x, "phil_pipe") ) {
    return &phil_pipe_main;
  }
...
void main_console() {
  while( 1 ) {
    ...
    else if( 0 == strcmp( cmd_argv[ 0 ], "dashboard" ) ) {
      puts(" dashboard" , 10);
      gui_on();
    } 

    else if( 0 == strcmp( cmd_argv[ 0 ], "dashboardoff" ) ) {
      puts(" dashboard off " , 15);
      gui_off();
    } 

    else if( 0 == strcmp( cmd_argv[ 0 ], "setprio" ) ) {
      int pid  = atoi( cmd_argv[ 1 ] );
      int prio = atoi( cmd_argv[ 2 ] );
      setPrio(pid, prio);
      puts( "priority changed " , 17);
    } 
    else {
