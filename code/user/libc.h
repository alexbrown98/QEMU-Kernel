...
#define SYS_PIPE_WR   ( 0x09 )
#define SYS_PIPE_RD   ( 0x10 )

#define SYS_SUSPEND   ( 0x12 )
#define SYS_RESTART   ( 0x13 )
#define SYS_PRIO      ( 0x14 )
#define SYS_GUI       ( 0x15 )
#define SYS_GUIOFF    ( 0x16 )
...
//sleep for s seconds
extern void sleep_s(int s);
//create a pipe 
extern int pipe(int fd[2]);
//write to pipe
extern int pipe_write(int fd, int data);
// read from pipe, set clear to 0 to leave data in pipe, or to 1 to erase the data
extern int pipe_read(int fd, int clear);

// same as pipe read but blocks if the pipe is empty. Tries 10 times before calling yield (to not use up processing power.)
extern int pipe_read_block(int fd, int clear);
// same as pipe write but blocks if the pipe is full. Tries 10 times before calling yield (to not use up processing power.)
extern int pipe_write_block(int fd, int data);
...
// turn on the terminal GUI
void gui_on();
// turn off the terminal GUI
void gui_off();
#endif
