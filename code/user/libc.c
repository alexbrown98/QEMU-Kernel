...
int pipe_write(int fd, int data) {
  int e;
  asm volatile ( "mov r0, %2 \n"
                 "mov r1, %3 \n"
                 "svc %1     \n"
                 "mov %0, r0" 
                :"=r" (e)
                :"I" (SYS_PIPE_WR), "r" (fd), "r" (data)
                :"r0", "r1");

  return e;
}

int pipe_read(int fd, int clear) {
  int e;
  asm volatile ( "mov r0, %2 \n"
                 "mov r1, %3 \n"
                 "svc %1     \n"
                 "mov %0, r0" 
                :"=r" (e)
                :"I" (SYS_PIPE_RD), "r" (fd), "r" (clear)
                :"r0");

  return e;
}

int pipe_read_block(int fd, int clear) {
  int i = 0;
  int data = pipe_read(fd, clear);
  while (data == -2) {
    if (i > 10) {
      i = 0; 
      yield();
      }
    data = pipe_read(fd, clear);
    i++;
  }
  pipe_write(fd, -2);
  return data;
...
int pipe_write_block(int fd, int data) {
  int i = 0;
  int data_ = pipe_write(fd, data);
  while (data_ == -2) {
    if (i > 10) {yield();}
    data_ = pipe_write(fd, data);
    i++;
  }
  return data_;
}
